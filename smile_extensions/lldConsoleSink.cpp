...
#include <iocore/lldConsoleSink.hpp>
#include <iostream>

#define MODULE "cLldConsoleSink"


SMILECOMPONENT_STATICS(cLldConsoleSink)

SMILECOMPONENT_REGCOMP(cLldConsoleSink)
{
  SMILECOMPONENT_REGCOMP_INIT

  scname = COMPONENT_NAME_CLLDCONSOLESINK;
  sdescription = COMPONENT_DESCRIPTION_CLLDCONSOLESINK;
  ...
  SMILECOMPONENT_MAKEINFO(cLldConsoleSink);
}

SMILECOMPONENT_CREATE(cLldConsoleSink)

//-----

cLldConsoleSink::cLldConsoleSink(const char *_name) :
  cDataSink(_name),
  fHandle(NULL),
  tick_no(0)
{
...
void cLldConsoleSink::fetchConfig()
{
...
/*
int cLldConsoleSink::myConfigureInstance()
{
...
int cLldConsoleSink::myFinaliseInstance()
{
...
int cLldConsoleSink::myTick(long long t)
{
...
cLldConsoleSink::~cLldConsoleSink()
{
  if (fHandle != NULL) {
    fclose(fHandle);
  }
