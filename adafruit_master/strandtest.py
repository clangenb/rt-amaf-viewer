...
#strip.setBrightness(150)                     # Limit brightness to ~1/4 duty cycle

# Runs 10 LEDs at a time along strip, cycling through red, green and blue.
# This requires about 200 mA for all the 'on' pixels + 1 mA per 'off' pixel.

head = 0               # Index of first 'on' pixel
tail = -100             # Index of last 'off' pixel
...
#	time.sleep(1.0 / 500000)                    # Pause 20 milliseconds (~50 fps)
#	print(color)
	head += 1
	if head >= numpixels:                   # Off end of strip?
		head = 0                            # Reset to start
		color >>= 8
		print(color)                         # Red->green->blue->black
		if color == 0:
