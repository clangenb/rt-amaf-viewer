...
static PyObject *DotStar_new(
  PyTypeObject *type, PyObject *arg, PyObject *kw) {
	...
	printf("Dotstar_new: start\n");

	switch(PyTuple_Size(arg)) {
	   case 4: // Pixel count, data pin, clock pin, bitrate
		if(!PyArg_ParseTuple(arg, "IbbI", &n_pixels, &dPin, &cPin, &bitrate))
			return NULL;
...
static PyTypeObject DotStarObjectType = {
	...
	.tp_name = "dotstar.Adafruit_DotStar", 
	.tp_itemsize = 0,
	.tp_basicsize = sizeof(DotStarObject),    
	.tp_dealloc = (destructor) DotStar_dealloc,
	.tp_flags = Py_TPFLAGS_DEFAULT,          		
	.tp_methods = methods,                     		
	.tp_init = (initproc) DotStar_init,      
	.tp_new = DotStar_new,            
};

static struct PyModuleDef dotstar = {
...
PyMODINIT_FUNC PyInit_dotstar(void) { // Module initialization function
	PyObject* m;
	printf("PyInit_dotstar: creating module...\n");

	m = PyModule_Create(&dotstar);
	printf("PyInit_dotstar: created pyModule\n");
	if (m == NULL) {
		printf("PyInit_dotstar: Could not create pyModule\n");
		return NULL;
	}

	if(PyType_Ready(&DotStarObjectType) < 0) {
	 	printf("PyInit_dotstar: Is not ready \n");
		return NULL;
	}
	printf("PyInit_dotstar: Is ready\n");
	
	Py_INCREF(&DotStarObjectType);
	printf("PyInit_dotstar: incremented ref.\n");
	PyModule_AddObject(m, "Adafruit_DotStar", (PyObject *)&DotStarObjectType);
